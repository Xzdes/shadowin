...
// ---- ИСПРАВЛЕНИЕ 2: Убираем неиспользуемый Cursor ----
// use std::io::Cursor;
// --------------------------------------------------
...
impl AiRenderer {
    ...
    pub async fn generate_image(&self, prompt: &str, width: u32, height: u32) -> Result<DynamicImage, Box<dyn std::error::Error>> {
        println!("AI Renderer: Sending prompt: '{}'", prompt);

        let request_body = Txt2ImgRequest {
            prompt: prompt.to_string(),
            negative_prompt: "blurry, worst quality, low quality, deformed".to_string(),
            steps: 20,
            width,
            height,
            cfg_scale: 7.0,
        ...
        let response_data: Txt2ImgResponse = response.json().await?;
