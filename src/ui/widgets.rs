...
pub struct Button<'a> {
    pub id: usize, pub x: i32, pub y: i32, pub width: u32, pub height: u32,
    pub state: ButtonState, text: String, font: &'a Font<'a>,
}

impl<'a> Button<'a> {
    pub fn new(id: usize, x: i32, y: i32, width: u32, height: u32, text: String, font: &'a Font) -> Self {
        Self { id, x, y, width, height, state: ButtonState::Idle, text, font }
    }
    
    // update теперь ТОЛЬКО для визуала
    pub fn update(&mut self, mouse_pos: (i32, i32), mouse_pressed: bool) {
        let is_over = self.is_over(mouse_pos);
        self.state = if is_over {
    ...
    pub fn is_over(&self, mouse_pos: (i32, i32)) -> bool {
        mouse_pos.0 >= self.x && mouse_pos.0 <= self.x + self.width as i32 &&
        mouse_pos.1 >= self.y && mouse_pos.1 <= self.y + self.height as i32
    }
    
    pub fn draw(&self, frame: &mut [u8], screen_width: u32) {
        let color = match self.state { ButtonState::Idle => [0, 123, 255, 255], ButtonState::Hovered => [0, 153, 255, 255], ButtonState::Pressed => [0, 93, 205, 255], };
        for row in 0..self.height { for col in 0..self.width { let px = self.x + col as i32; let py = self.y + row as i32; if px >= 0 && py >= 0 { let index = ((py as u32 * screen_width) + px as u32) as usize * 4; if index + 3 < frame.len() { frame[index..index + 4].copy_from_slice(&color); } } } }
        let scale = Scale { x: 30.0, y: 30.0 }; let text_color = [255, 255, 255, 255]; let v_metrics = self.font.v_metrics(scale); let text_height = v_metrics.ascent - v_metrics.descent;
        ...
        let text_start_x = self.x + ((self.width as f32 - text_width) / 2.0) as i32; let text_start_y = self.y + ((self.height as f32 + text_height) / 2.0) as i32;
